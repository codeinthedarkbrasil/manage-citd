...
export async function getParticipants(_event: string) {
  return participants
}

export async function setSelectedParticipants(ids: string[]) {
  participants.filter(p => p.selected).forEach(p => { p.selected = false })
